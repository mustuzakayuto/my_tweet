            ...
            <div>
                ...
                <select class="form-control" name="year">
                    <?php foreach (range(1900, date('Y')) as $year) : ?>
                        <option value="<?= $year ?>" <?= selected($year, $regist['year'] ?? 1900) ?>><?= $year ?></option>
                    <?php endforeach ?>
                </select>
                <select class="form-control" name="month">
                    <?php foreach (range(1, 12) as $month) : ?>
                        <option value="<?= $month ?>" <?= selected($month, $regist['month'] ?? 1) ?>><?= $month ?></option>
                    <?php endforeach ?>
                </select>
                <select class="form-control" name="day">
                    <?php foreach (range(1, 31) as $day) : ?>
                        <option value="<?= $day ?>" <?= selected($day, $regist['day'] ?? 1) ?>><?= $day ?></option>
                    <?php endforeach ?>
                </select>
            </div>
            <div>
